...
## Step 0 â€” Detect the repo's conventions before imposing any
...
# Actual default branch â€” never assume "main"
...
6. **Never merge with unresolved review threads or failing required checks** unless the user explicitly overrides â€” and then note the override in the PR.
7. **Never commit secrets, credentials, or tokens** â€” and never work around a blocked push by disabling secret scanning. Rotate anything that leaked.
...
9. **No editorializing in commits, PRs, or review replies.** State what changed and why â€” not how good it is. Never add AI attribution/signatures unless the repo requires it.

## Task routing

Load the reference for the task at hand â€” don't load all of them:

| You are about toâ€¦ | Read |
|---|---|
...
# 5. Watch CI â€” use native watchers, never hand-rolled poll loops
...
- **Verify your branch after any detour.** Switching to another PR mid-task and forgetting to switch back is the classic way edits land on the wrong branch. `git branch --show-current` costs nothing â€” run it before resuming edits and before staging. If work landed wrong: `git stash push -u -m "misplaced" -- <paths>` (`-u` so new files come along), checkout the right branch, `git stash pop`.
...
- **One PR = one worktree when working several in parallel** (`git worktree add ../repo-pr42 branch-name`) â€” eliminates cross-PR contamination entirely.
- **Link work to issues** (`Closes #123` in the PR body) so project tracking updates automatically on merge.
- **When the user's request is ambiguous about a public action's wording** (review tone, PR title, merge strategy), show a draft and ask â€” don't guess and publish.

## Platform note: Windows / PowerShell

Every command in this skill is written for a POSIX shell (bash/zsh). Two PowerShell-specific traps to know about:

- **JSON payloads for `gh api --input <file>`**: PowerShell's `Out-File -Encoding utf8` writes a UTF-8 **BOM**, which `gh api` fails to parse (`Problems parsing JSON`) with no indication the BOM is the cause. Write JSON payloads without a BOM instead:
  ```powershell
  [System.IO.File]::WriteAllText("$PWD\payload.json", $json, (New-Object System.Text.UTF8Encoding($false)))
  ```
  or use `Set-Content` (BOM-free by default in PowerShell 6+) rather than `Out-File`.
- **Heredocs**: bash `<<'EOF'` blocks and `2>&1 </dev/null` redirections in this skill's examples don't translate â€” use PowerShell here-strings (`@"..."@`) and `Start-Process`/native redirection instead.
...
Never amend or rebase commits that are already on a shared branch others may have pulled â€” revert instead.
